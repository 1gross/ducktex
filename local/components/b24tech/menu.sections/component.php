...
if ($this->StartResultCache()) {
    if (!CModule::IncludeModule("iblock")) {
        ...
        $applyFilter = [];
        if (isset($arParams["FILTER_NAME"]) && isset($GLOBALS[ $arParams["FILTER_NAME"] ])) {
            $applyFilter = $GLOBALS[ $arParams["FILTER_NAME"] ];
        ...
        while ($arSection = $rsSections->GetNext()) {
            if (isset($GLOBALS["HIDE_ELEMENT_CNT_ZERO"]) && intval($arSection["ELEMENT_CNT"]) == 0) {
                continue;
            }
            $arResult["SECTIONS"][] = array(
                "ID" => $arSection["ID"],
                ...
                "MENU_IMAGE" => $arSection["UF_MENU_IMAGE"],
                "MENU_IMAGE_HOVER" => $arSection["UF_MENU_IMAGE_HOVER"],
                "CATALOG_SECTION_LINK" => $arSection["UF_CATALOG_SECTION"],
...
foreach ($arResult["SECTIONS"] as $arSection) {
    ...
    $aMenuLinksNew[ $menuIndex++ ] = array(
        ...
        array(
            ...
            "IMAGE" => $arSection["MENU_IMAGE"],
            "IMAGE_HOVER" => $arSection["MENU_IMAGE_HOVER"],
            "CATALOG_SECTION_LINK" => $arSection["CATALOG_SECTION_LINK"],
            "FROM_IBLOCK" => true,
            "IS_PARENT" => false,
            "DEPTH_LEVEL" => $arSection["DEPTH_LEVEL"],
            "APPLY_FILTER" => $arSection["APPLY_FILTER"],
        ),
    );
}
